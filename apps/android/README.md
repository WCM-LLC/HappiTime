...
## Correction to the record (2026-08-14)

PR #177 aligned this workspace's dependency list with the packages
`apps/mobile` code imports, and its commit message claimed the Happy Hour scan
"has never worked on Android" and was "broken since May". **That claim is
false.** A Play beta build completed a scan end to end on 2026-08-14 —
`expo-image-picker` and `expo-image-manipulator` were both linked and working.

The reasoning error is worth keeping, because it is easy to repeat: Expo
autolinking discovers native modules by scanning `node_modules` search paths,
**not** by reading the build workspace's declared dependencies. On a clean
install the package hoists to the monorepo root, where autolinking finds it no
matter which workspace declared it. The original diagnosis generalised from a
single laptop's `node_modules` layout, where those packages happened to sit
nested under `apps/mobile/node_modules`.

What #177 actually delivered, and what still stands:

- Declared dependencies now match the code this workspace ships. Good hygiene.
- Five version mismatches aligned, per `npx expo install --check`, which is
  authoritative — `react-native-maps` belongs at `1.20.1`, not `^1.27.2`.
- `test/mobile-android-dependency-parity.test.mjs` guards that manifest against
  future drift. Read a failure there as "the manifest drifted from the code",
  not as "the Android build is broken".

Note also that both `apps/mobile/android` and `apps/android/android` exist
locally and are generated by `expo prebuild`; neither is tracked in git, and
both carry the same `com.jwill7486.happitime.mobile` namespace. Whichever
workspace you run the build from generates its own.
