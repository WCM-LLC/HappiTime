...
## Phase 2 — native open (needs Apple Team ID + rides the Apple-nonce EAS build)
**Blocked on: Apple Team ID** (for the AASA `appID`). Then:
1. **`apps/directory/public/.well-known/apple-app-site-association`** (no extension, served
   as `application/json`): `{ "applinks": { "details": [{ "appID": "<TEAMID>.com.jwill7486.happitime.mobile", "paths": ["/i/*"] }] } }`.
   Add `assetlinks.json` for Android (package `com.jwill7486.happitime.mobile` + release SHA-256).
   Confirm Next serves `/.well-known/*` (static `public/` files are served as-is).
...
3. **Mobile deep-link handler** (extend `useMagicLinkListener` or new `useItineraryDeepLink`):
   match `https://happitime.biz/i/<token>` + `happitime://itinerary?token=<token>` →
   `get_shared_itinerary(token)` → navigate to `ItineraryDetail`.
4. **Build + device test (HARD GATE):** same EAS build as the Apple nonce. On a real iPhone
   with the app installed, tapping a shared SMS link opens the app **to the itinerary**;
   also run the Apple-login test. Do not release without both.
