...
- ~~Supabase `auth_rls_initplan` warnings: wrap repeated `auth.uid()` and role helper calls in scalar subqueries such as `(select auth.uid())`.~~ Done 2026-08-11 (PR #147): all 97 policies rewritten via generated migration; advisor re-scan shows 0 initplan warnings. Still open from the advisor set: `multiple_permissive_policies` (173) and `duplicate_index` (3).
...
- ~~Next.js image warnings: replace remaining raw `<img>` usage with `next/image` where appropriate.~~ Directory done 2026-08-11: VenueCard, venue-detail gallery, and KCMapPage hero now use `next/image` with a custom Cloudinary loader (`src/lib/imageLoader.ts` + `images.loaderFile`) so srcsets are served by Cloudinary at no Vercel image-optimization cost. Two documented keeps: ImageLightbox (fullscreen, pre-sized CDN variant) and StoreDownloadCTA QR (external SVG). `apps/web` `<img>` usage (e.g. VenueMediaUploader) not in this batch.
- ~~Mobile lint warnings: fix the `session` hook dependency in `apps/mobile/App.tsx` and remove/use the unused `navigation` variable in `InviteScreen.tsx`.~~ Done 2026-08-11: all 5 mobile lint warnings cleared (`App.tsx` hook dep, unused eslint-disable in `useCheckin.ts`, unused `getPlaceTagSlugs` in `HomeScreen.tsx`, unused `navigation` in `InviteScreen.tsx`, unused `View` import in `onboarding/atoms.tsx`); `npm run lint --workspace mobile` is clean.
- Directory lint setup (added 2026-08-11): `apps/directory/.eslintrc.json` now exists (`next/core-web-vitals`) so `next lint` runs non-interactively. Same day: all 48 flagged `<a>`→`<Link>` conversions done across 13 files and `@next/next/no-html-link-for-pages` restored to its default error level; lint exits 0. Remaining warnings: `no-img-element` (5), `exhaustive-deps` (2), `next-script-for-ga` (2), `no-page-custom-font` (1).
