...
test("parses the https Universal Link form", () => {
  assert.deepEqual(parseItineraryLink(`https://happitime.biz/i/${TOKEN}`), { token: TOKEN, ref: null });
});
...
test("parses a subdomain host", () => {
  assert.deepEqual(parseItineraryLink(`https://www.happitime.biz/i/${TOKEN}`), { token: TOKEN, ref: null });
});

test("parses the custom-scheme form", () => {
  assert.deepEqual(parseItineraryLink(`happitime://itinerary?token=${TOKEN}`), { token: TOKEN, ref: null });
});
